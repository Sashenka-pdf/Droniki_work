...
int main(void) {
    ...
    /* Coordinate Logging */
    //Log drone coordinates every second during flight
    while (true) {
        int32_t latitude = 0, longitude = 0, altitude = 0;
        if (getCoords(latitude, longitude, altitude)) {
            // Convert from API format to human-readable format
            // Latitude and longitude: from degrees * 10^7 to degrees
            // Altitude: from cm to meters
            double lat_deg = latitude / 10000000.0;
            double lon_deg = longitude / 10000000.0;
            double alt_m = altitude / 100.0;
            
            snprintf(logBuffer, 256, "Coordinates - Lat: %.7f, Lon: %.7f, Alt: %.2f m", 
                     lat_deg, lon_deg, alt_m);
            logEntry(logBuffer, ENTITY_NAME, LogLevel::LOG_INFO);
        } else {
            logEntry("Failed to get coordinates from Navigation System", ENTITY_NAME, LogLevel::LOG_WARNING);
        }
        sleep(1);
    }
